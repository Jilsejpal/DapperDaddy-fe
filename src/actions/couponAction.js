...
export const applyCoupon = (couponCode, totalPrice) => {
  return async (dispatch) => {
    try {
      ...
      // Make an API call to apply the coupon using AxiosInstanceaxiosInstance
      const { data } = await axiosInstance.post("/api/v1/applycoupon", {
        couponCode,
        totalPrice,
      });
...
export const deleteCoupon = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.delete(`/api/v1/admin/coupon/${id}`);
